...
== Branch Protection
Our `main` branch is protected, meaning that no direct pushes to this branch are allowed. All changes to the `main` branch must be made through Pull Requests. The purpose of this is to allow an AI assistant to review the changes before they are merged, ensuring that any potential issues are caught and resolved early. This ensures that all modifications are reviewed before they are integrated into the `main` codebase
