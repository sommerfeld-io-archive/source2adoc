...
[source, bash]
....
docker run --volume "$(pwd):$(pwd)" --workdir "$(pwd)" sommerfeldio/source2adoc:latest --source-dir src --output-dir docs --exclude path/inside/src/dir --exclude path/inside/src/dir/script.sh
....
