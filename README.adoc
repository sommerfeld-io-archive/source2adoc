...
....
                                                         +---------------------+                  +---------------------+
                                                         |  CodeFileInventory  |           +------|  Parser (and Lint)  |
+---------------------+       +-------------+            |                     |           |      +---------------------+
|  FileSystemScanner  +------>| CodeFile    +-------------> List of CodeFile --------------+------|  Metadata           |
+---------------------+  new  |             |  register  |                     |  fetch    |      +---------------------+
                              |       type  |  CodeFile  +---------------------+  single   +------|  ContentGenerator   |
                              |       path  |                                              |      +---------------------+
                              |   filename  |                                              +------|  DocFileWriter      |
                              +-------------+                                                     +---------------------+
....
...
. find all code files
. store in List or stack or some sort of event bus / event thing
. parse all file and store somewhere
. get metadata for each code file and store somewhere (filename, path, ...)
. generate content for each docs file and store somewhere
. write docs files to filesystem
. generate single `nav.adoc` for all code files
====
